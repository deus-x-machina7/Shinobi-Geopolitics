...
import { ArrowLeft, Users, Coins, TrendingUp, Play, Pause, RotateCcw, BarChart3, TrendingUp as TrendingUpIcon, CloudRain, History } from 'lucide-react';
import { motion } from 'framer-motion';

// Order ensures Rain renders last (on top) in maps, but we will sort specifically for render order anyway
const COMPARISON_NATIONS = ['Fire', 'Lightning', 'Earth', 'Wind', 'Water', 'Rain'];
...
const CustomTooltip = ({ active, payload, label, currencyMode, isCurrency }: any) => {
  if (active && payload && payload.length) {
    // Sort payload so Rain is at top
    const sortedPayload = [...payload].sort((a, b) => {
        if (a.name === 'Rain') return -1;
        if (b.name === 'Rain') return 1;
      ...
      <div className="bg-slate-900/95 border border-slate-700 p-4 rounded-lg shadow-2xl backdrop-blur-md z-50">
        ...
        <div className="space-y-1">
          {sortedPayload.map((entry: any) => (
            <div key={entry.name} className={`flex justify-between gap-4 text-xs font-mono ${entry.name === 'Rain' ? 'bg-white/10 p-1 rounded -mx-1' : ''}`}>
              <span style={{ color: entry.color, fontWeight: entry.name === 'Rain' ? 'bold' : 'normal' }}>{entry.name}:</span>
...
const BarRaceChart = ({ metricPrefix, scope, isCurrency, currencyMode }: any) => {
  ...
  const currentData = useMemo(() => {
    return COMPARISON_NATIONS.map(nation => ({
      name: nation,
                    ...
                    <Bar dataKey="value" animationDuration={500} radius={[0, 4, 4, 0]}>
                        {currentData.map((entry, index) => (
                            <Cell 
                                key={`cell-${index}`} 
                                fill={entry.color}
                                fillOpacity={entry.name === 'Rain' ? 1 : 0.4} 
                                stroke={entry.name === 'Rain' ? '#fff' : 'none'}
                                strokeWidth={entry.name === 'Rain' ? 1 : 0}
                            />
                        ))}
                    </Bar>
                </BarChart>
...
const ComparisonSection = ({ title, icon: Icon, metricPrefix, isCurrency = false }: any) => {
  ...
  // Ensure Rain renders on top by placing it last in the Line array
  const sortedNations = [...COMPARISON_NATIONS].sort((a, b) => a === 'Rain' ? 1 : b === 'Rain' ? -1 : 0);
                    ...
                    {sortedNations.map(nation => {
                        ...
                        return (
                            <Line 
                                ...
                                strokeWidth={isRain ? 4 : 2}
                                strokeOpacity={isRain ? 1 : 0.3}
                                dot={isRain ? {r: 3, strokeWidth: 2, fill: '#0f172a'} : false}
                                activeDot={{ r: isRain ? 8 : 6 }}
...
const WarEraComparison = () => {
  ...
  const SNAPSHOTS = [
    { year: 16, label: "First War Peak" },
    { year: 35, label: "Eve of Second War" },
    { year: 36, label: "Second War Destruction" },
    ...
    <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-6 backdrop-blur-sm mt-8">
       ...
       <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
          <div className="flex items-center gap-3">
             ...
             <h2 className="text-xl font-serif text-slate-100">Conflict Impact Assessment</h2>
          </div>
    ...
    <div className="min-h-screen bg-void pb-24 pt-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-12">
          ...
          >
            ...
            <h1 className="text-4xl md:text-6xl font-serif text-white tracking-tight">
                Rain <span className="text-slate-600">Anomaly</span>
            </h1>
          </motion.div>
          <p className="text-slate-400 max-w-2xl font-light">
            Asymmetric comparative analysis. Visualizing the extreme volatility and "Boom/Bust" cycles of Rain Country relative to the stability of the Great Nations.
          </p>
        </div>
        ...
        <div className="space-y-12">
          ...
          <ComparisonSection 
            title="Quality of Life (Per Capita)" 
            icon={TrendingUp} 
...
export default RainComparisonDashboard;
